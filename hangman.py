...
def play(word):
    ...
    mode = input('Выберите сложность игры: \n Cредняя - будут показаны первая и последняя буквы загаданного слова; \n Высокая - первая и последняя буквы показаны не будут; \n Введите "средняя" или "высокая": ').strip()
    if mode.lower() == 'средняя':
        word_completion = word[0] + '_' * (len(word) - 2) + word[-1]
    print(display_hangman(tries))
    print(word_completion)
    print(f'Количество букв в слове {len(word)}')
    while not guessed and tries != 0:
        attempt = input('Введите букву или слово целиком:').upper()
        if len(attempt) == 1:  # если пользователь ввел букву
            if attempt.isalpha():
                if attempt not in guessed_letters:
                    guessed_letters += attempt
                    if attempt in word:  # если пользователь угадал букву
                        ...
                        if word_completion.count('_') == 0:
                            ...
                            guessed = True
                            break
                    elif attempt not in word:  # если пользователь не угадал букву
                        tries -= 1
                        print('Данная буква не представлена в слове.')
                        print(display_hangman(tries))
                        print(word_completion)
                else:
                    print('Вы ввели букву, которую вводили ранее. Попытка не засчитана.')
                    continue
            else:
                print('Вы ввели символ, не являющийся буквой. Попытка не засчитана.')
                continue
        elif len(attempt) > 1:  # если пользователь ввел слово
            if attempt not in guessed_words:
                if attempt == word:  # если пользователь угадал слово
                    print('Поздравляем, вы угадали слово! Вы победили!')
                    guessed = True
                    break
                elif attempt != word:
                    tries -= 1
                    print('Нет, было загадано другое слово.')
                    print(display_hangman(tries))
                    print(word_completion)
            elif attempt in guessed_words:
                print('Вы ввели слово, которое вводили ранее. Попытка не засчитана.')
                continue
    else:
        print(f'Было загадано слово {word}.')
while True:
    answer = input('Хотите ли Вы начать новую игру? (д - да, н - нет):').strip()
    if answer.lower() == 'д':
        word = get_word(word_list)
        play(word)
    elif answer.lower() == 'н':
        print('Всего доброго! Скоро увидимся...')
        break
